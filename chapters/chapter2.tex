...
\begin{conjecture}[Erd\H{o}s--Faber--Lov\'asz]
  Sei $n\in \N$. F"ur alle Graphen $G\in\cE(n)$ ist $\chi(G) \leq n$.
\label{con:efl}
\end{conjecture}
...
\begin{conjecture}[Erd\H{o}s--Faber--Lov\'asz]
  Sei $H$ ein linearer Hypergraph mit $|H| = n \in \N$. Dann gilt $\chi'(H) \leq |H|$.
\label{con:eflhyper}
\end{conjecture}
...
\begin{theorem}
  Ist $n\in\N$ und $G\in \cE(n)$, so gilt: $\chi(G) \leq \frac{3n}{2}-2$.
...
Diese Art der "Uberdeckung wurde zuerst von Krausz \cite{Krausz43} zur Charakterisierung von Kantengraphen verwendet, daher der Name Krauszzerlegung. 
  ...
  \end{enumerate}
  Desweiteren sei f"ur $v\in V(G)$ der \DF{Grad} von $v$ bez"uglich $\mathcal K$ definiert als $$d_{\mathcal{K}}(v) = |\{ K\in\mathcal K \;|\;  v \in V(K)\}|,$$ der \DF{Minimalgrad} von $G$ bez"uglich $\mathcal K$ als $$\delta(\mathcal K) = \min\limits_{v\in V(G)}d_{\mathcal{K}}(v)$$ und der \DF{Maximalgrad} von $G$ bez"uglich $\mathcal{K}$ als $$\Delta(\mathcal{K}) = \max\limits_{v\in V(G)}d_{\mathcal{K}}(v).$$
...
Somit l"asst sich der Kantengraph $L(G)$ mit vollst"andigen Graphen "uberdecken. Die Krauszzerlegungen sind eine Verallgemeinerung dieses Konzepts.
...
\begin{proof}
  ...
  Somit reicht es zu zeigen, das $\chi(H) \leq p$. Ist $V(H) =\varnothing$, so gilt dies trivialerweise. Andernfalls gilt nach Konstruktion $\delta(H) \geq p $. 
