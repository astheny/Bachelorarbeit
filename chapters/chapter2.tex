...
\begin{proof}
  ...
  Sei nun $\delta(G) = d$. Wir m"ussen zeigen, dass es eine Krauszzerlegung \mathcal{K} gibt, mit $d_{G}(v:\mathcal{K}) \geq d$. Sei $E(G)= \{e_1\dots e_{j}\}$ eine Nummerierung der Kanten. Setze $H^{i}= G[e_i]$. Dann ist $\mathcal{K} = \{H^{1}\dots H^{j}\}$ eine Krauszzerlegung von $G$ mit $d_{G}(v:\mathcal{K} \geq \delta(G)$ f"ur alle $v\in V(G)$. Also ist $\kappa_d(G) <\infty$.
