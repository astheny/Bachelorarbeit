...
$K_{k:1}$ ist ein vollst"andiger Graph der Ordnung $k$ und $K_{2r-1:r}$ besitzt keine Kanten. 
...
\end{theorem}
Mit den beiden vorrangegangenen S"atzen k"onnen wir nun einen Satz "uber die Eigenwerte von Kneser Graphn beweisen.
\begin{proposition}
...
\begin{proof}
  ...
  \ncase{3} {$ k = 2r $} Die Ecken von $G$ sind alle $r$-elementingen Teilmengen von $\left\{ 1,\dots,k \right\}$. Da $k=2r$ ist f"ur ein $w\in V(G)$ die einzige benachbarte Ecke ihr Komplement in $\left\{ 1,\dots,k \right\}$. Also sind die Komponenten von $G$ alle isomorph zu $K_{2}$. Dann ist $\chi(G) = 2$ und $\omega(G) = 2$. 
...
\begin{proof}
  Da $G$ ein perfekter Graph ist, gilt $k = \chi(G) = \omega(G)$. 
...
\begin{proof}
  ...
  \ncase{3}{k = 3} Ist $|G| \geq 7$, so ist wegen Satz \ref{prop:planaregraphen} nichts zu zeigen. Anderfalls ist $|G| \leq 6$. Da $G$ nicht bipartit ist, besitzt $G$ nach dem Satz von K"onig einen ungeraden Kreis $C$ als Untergraphen. Da $|G| \leq 6$ gilt dann $|C| = 3$ oder  $|C| = 5$.
...
\end{theorem}
\begin{proof}
  Gilt $\chi(G) + \chi(\overline{G}) \leq n$, so ist nach Satz \ref{thm:xikomplement} nichts mehr zu zeigen. Anderfalls ist $G$ entweder vom Typ $F_1$ oder $F_2$. 
