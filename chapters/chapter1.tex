...
Es sei $G=(V(G),E(G))$ ein beliebiger Graph. Eine \DF{(Ecken-)F"arbung} von $G$ zur \DF{Farbmenge} $C$ ist eine Abbildung $f:V(G)\to C$ mit $f(u)\neq f(v)$ f"ur alle $u,v \in V(G)$. Ist $\left|C\right|=k \in \N$, so heißt $f$ \DF{$k$-F"arbung}. Die \DF{chromatische Zahl} $\chi(G)$ ist die kleinste Zahl $k\in\N$, sodass $G$ eine $k$-F"arbung besitzt. \\
Eine \DF{Kantenf"arbung} zur \DF{Farbmenge} $C$ ist eine Abbildung $f:E(G)\to C$ mit $f(e)\neq f(e')$ falls $e,e'\in E(G)$ eine gemeinsame Endecke besitzen.
...
Es sei $A\in \Rnn$ eine symmetrische Matrix. Dann hat $A$ nur reelle Eigenwerte und folglich k"onnen dieses fallend angeordnet werden. F"ur eine symmetrische Matrix $A$ sei also $\lambda_i(A)$ der i-gr"oßte \DF{Eigenwert} (gez"ahlt mit Vielfachheiten). Eine symmetrische Matrix $A$ hei"st \DF{positiv semidefinit} falls $x^TAx \geq 0$ f"ur alle $x \in \R^n$. Gilt $x^TAx = 0$ nur f"ur $x = 0$(also $x^{T}Ax > 0$ f"ur $x\neq 0$), so hei"st $A$ \DF{positiv definit}. Wir wollen nun einige Eigenschaften von positiv (semi)definiten Matrizen anf"uhren.
\begin{proposition}
...
\begin{proof}
  Seien $l_1<\dots< l_{n-k}$ die Nummern der Zeilen bzw. Spalten die nach dem L"oschen der Zeilen bzw. Spalten von $A$ "ubrig bleiben. \todo{bessere Formulierung} Setze $P:=(e_{l_1},e_{l_2},\dots,e_{l_{n-k}})\in \R^{n\times n-k}$. Dann ist $B=P^{T}AP$. Es folgt mit Hilfe von \ref{thm:CourantFischer} : 
  \begin{align*}
    \mu_i &= \max\{\min\limits_{x\in V , x \neq 0} \frac{x^TBx}{x^Tx}| V\subseteq \R^n \text{ ist linearer Unterraum der Dimension } i\} \\
    &= \max\{\min\limits_{x\in V , x \neq 0} \frac{x^TP^{T}APx}{x^Tx}| V\subseteq \R^n \text{ ist linearer Unterraum der Dimension } i\} \\
    &\leq \max\{\min\limits_{y\in PV , y \neq 0} \frac{y^{T}Ay}{y^Ty}| V\subseteq \R^n \text{ ist linearer Unterraum der Dimension } i\} \\
    &\leq \max\{\min\limits_{x\in V , x \neq 0} \frac{y^TAy}{y^Ty}| V\subseteq \R^n \text{ ist linearer Unterraum der Dimension } i\} \\
    &= \lambda_i
  \end{align*}
  \todo{PV anders}
\end{proof}
...
Sei dazu zun"achst $G=K_n$ der vollst"andige Graph der Ordnung $n$. Dann ist $A(G)=J-I$, wobei $J\in\Rnn$ die Matrix ist, bei der jeder Eintrag $1$ ist, und $I\in\Rnn$ die Einheitsmatrix. Somit ist $-1$ ein Eigenwert mit Vielfachheit $n-1$. Außerdem folgt aus \ref{rem:evGraph}, dass $n-1$ ein Eigenwert von $G$ ist. \\
Sei nun $G=C_n$
\todo{vollst"andige Graphen, Kreise, Komplemente usw. Bilder!}
