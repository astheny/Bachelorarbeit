...
In der folgenden Arbeit seien die nat"urlichen Zahlen $\N$ stets die Menge $ \left\{ 1, 2 , \dots \right\}$.
...
Gibt es eine Menge von Ecken $X\subseteq V(G)$ mit $V(H) = X$ und $E(H) = \left\{ v_iv_j|v_i,v_j\in V(H) \text{ und } v_iv_j \in E(G) \right\}$ so heißt $H$ \DF{induzierter Untergraph} von $G$. Wir bezeichnen $H$ dann mit $G[X]$.
Die \DF{Cliquenzahl} $\omega(G)$ ist die gr"o{\ss}te Zahl $k$, sodass $G$ den vollst"andigen Graph der Ordnung $k$  als Untergraphen enth"alt, die \DF{Unabh"angigkeitszahl} $\alpha(G)$ ist die gr"oßte Zahl $k$, sodass $G$ den leeren Graphen der Ordnung $k$ als Untergraphen enth"alt.

Ein (schlingenloser) Hypergraph $H=(V(H),E(H))$ mit \DF{Eckenmenge} $V(H)$ und \DF{Kantenmenge} $E(H)$ heißt \DF{linearer Hypergraph}, falls zwei verschiedene Kanten $e,e'\in E(H)$ maximal eine Ecke gemeinsam haben ($|e\cap e'| \leq 1$). Die \DF{Ordnung} von $H$ sei $|V(H)|$ und wird mit $|H|$ bezeichnet. F"ur eine Kante $e$ von $H$ ist  der \DF{Kantengrad} die Anzahl aller Kanten, die mit $e$ einen nichtleeren Schnitt haben. Der \DF{Eckengrad} einer Ecke $v \in V(H)$ ist
...
Eine \DF{F"arbung} von $G$ ist eine Abbildung $f:V(G)\to C$ mit $f(v)\neq f(w)$ f"ur alle $vw\in E(G)$, wobei $C$ eine beliebige Menge, die \DF{Farbmenge}, ist. Ist $|C| = k$ so heißt $f$ \DF{k-Kantenf"arbung}. Die \DF{chromatische Zahl} $\chi(G)$ bezeichne die kleinste postivie Zahl $k$, f"ur welche eine $k$-F"arbung von $G$ existiert. 
...
\end{proof}
F"ur einen Graphen $G$ seien die \DF{Eigenwerte} von $G$ definiert als $\lambda_{p}(G)  = \lambda_{p}(A(G))$ f"ur alle $1 \leq p \leq n$. 
\begin{remark}
  \label{rem:evGraph} 
...
Sei dazu zun"achst $G=K_n$ . Dann ist $A(G)=J-I$, wobei $J\in\Rnn$ die Matrix ist, bei der jeder Eintrag $1$ ist, und $I\in\Rnn$ die Einheitsmatrix. Somit ist $-1$ ein Eigenwert mit Vielfachheit $n-1$. Da $K_n$ $n-1$ regul"ar ist, ist der Vektor $(1,1,\dots,1)^{T}$ ein Eigenvektor von $G$ zum Eigenwert $n-1$.

Sei nun $G=C_n$. Dann hat $G$ die Eigenwerte $2 \cos (\frac{2\pi p}{n})$ f"ur $p\in \left\{ 1,\dots,n \right\}$ (siehe z.B. \cite[1.1.4]{zbMATH05625877}).
\todo{Bilder!}
